...
import { ChatMode, LLMServer, EmbeddingServer } from '../main';
import {i18n, getChatModeRecords} from '../config';
import { fragWithHTML } from '../utils/Utils'

const SUPPORT_MODELS = ["gpt-3.5-turbo", "gpt-4", "gpt-4-turbo", "gpt-4o"]
const EMBEDDING_SUPPORT_MODELS = ["text-embedding-3-small", "text-embedding-3-large", "text-embedding-ada-002"]
				...
				.addText(text => {
					...
					text.inputEl.onblur = () => {
						...
						if (!SUPPORT_MODELS.includes(model)) {
							new ModelCheckModal(this.app, SUPPORT_MODELS).open();
						}
					}
				});
		...
		if (embeddingServer == EmbeddingServer.OpenAI) {
			...
			let s = new Setting(containerEl)
				...
				.addText(text => {
					text
						.setPlaceholder('MODEL NAME')
						.setValue(this.plugin.settings.openaiConfig.embedding_model ?? '')
						.onChange(async (value) => {
							this.plugin.settings.openaiConfig.embedding_model = value;
							await this.plugin.saveSettings();
							this.showEmptyInputError(text.inputEl)
					...
					// 失去焦点，检测值的正确性
					text.inputEl.onblur = () => {
						let model = this.plugin.settings.openaiConfig.embedding_model ?? ""
						if (!EMBEDDING_SUPPORT_MODELS.includes(model)) {
							new ModelCheckModal(this.app, EMBEDDING_SUPPORT_MODELS).open();
						}
					}
				});

			let embedding_model = this.plugin.settings.openaiConfig.embedding_model
			if (!embedding_model) {
				// containerEl.inputEl.addClass('error');
				let inputEl = s.controlEl.getElementsByTagName("input")[0]
				this.showEmptyInputError(inputEl)
		...
		if (embeddingServer == EmbeddingServer.Ollama) {
			...
			let s = new Setting(containerEl)
				...
				.addText(text => {
					text
						.setPlaceholder('MODEL NAME')
						.setValue(this.plugin.settings.ollamaConfig.embedding_model ?? '')
						.onChange(async (value) => {
							this.plugin.settings.ollamaConfig.embedding_model = value;
							await this.plugin.saveSettings();
